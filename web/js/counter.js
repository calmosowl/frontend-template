...
function WidgetTopPanel(options) {
	...
	this.update = function (json) {
		...
		for (var counter in jsonData) {
			...
			if (id && !that.counterBuffer.has(id)) that.counterBuffer.set(jsonData[counter].id, new JackPotCounter(jsonData[counter]));
...
function JackPotCounter(options) {
	...
	this.transform = function (elemArr, val) {
		...
		for (var i = arr.length; i >= 0; i--) {
			...
			if (_this.coordinates.rBuffer[i] > 0 && _this.coordinates.rBuffer[i] < 360) {
				var bezier = 'cubic-bezier(.23,1,.32,1)';
				that.setTransform(arr[i - 1], rotate, 1000 + _this.coordinates.delay, +_this.coordinates.duration - 1000, bezier);
			}
			if (_this.coordinates.rBuffer[i] > 359) {
				var _bezier = 'cubic-bezier(.7, .16, .3, .84)';
				that.setTransform(arr[i - 1], rotate, _this.coordinates.duration, _this.coordinates.delay, _bezier);
			}
		}
	};
